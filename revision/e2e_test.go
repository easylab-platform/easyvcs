...
// TestE2E_MultiParentRebaseConflictResolve exercises the full revision-native
// flow: commit a base, create two divergent snapshots, rebase a revision onto
// both parents (multi-parent, jj-style merge), which yields a first-class
// conflict; then resolve it while keeping the stable revision id.
func TestE2E_MultiParentRebaseConflictResolve(t *testing.T) {
	runBoth(t, func(t *testing.T, ws *Workspace, dir string) {
		...
		if ch.ID != oursSnap.RevisionID {
			t.Fatalf("revision id changed on rebase: %s != %s", ch.ID, oursSnap.RevisionID)
		}
		...
		// Resolve to side 0 (ours). revision id stays stable; conflict removed.
		rs, rch, err := ws.Resolve(oursSnap.RevisionID, "o.txt", 0)
		if err != nil {
			t.Fatal(err)
		}
		if rch.ID != oursSnap.RevisionID {
			t.Fatalf("revision id changed on resolve: %s != %s", rch.ID, oursSnap.RevisionID)
		}
		...
		t.Logf("e2e multi-parent rebase + resolve ok: revision=%s stable, content=%s", shortID(rch.ID), string(blob))
	})
}
...
func TestResolvePropagatesToDescendants(t *testing.T) {
	runBoth(t, func(t *testing.T, ws *Workspace, dir string) {
		...
		// Parent revision carries the conflict.
		...
		// Child revision on top of parent inherits the conflict at the same path.
		...
		if rch.ID != parentCh.ID {
			t.Fatalf("parent revision id changed on resolve: %s", rch.ID)
		}
		...
		if newChild.ID != childCh.ID {
			t.Fatalf("child revision id changed: %s -> %s", childCh.ID, newChild.ID)
		}
